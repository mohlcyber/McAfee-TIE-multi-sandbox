...
class ATD():
    ...
    def _auth(self, creds):
        try:
            ...
            res = requests.get(self.url + "session.php", headers=sessionheaders, verify=self.verify)

            if res.status_code == 200:
                results = res.json()['results']
                tmp_headers = results['session'] + ':' + results['userId']

                headers = {'VE-SDK-API': base64.b64encode(tmp_headers.encode()),
                           'Accept': 'application/vnd.ve.v1.0+json',
                           'accept-encoding': 'gzip;q=0,deflate,sdch'}
                print('ATD STATUS: Successful authenticated {0}.'.format(ATD_USER))
                return headers
            else:
            ...
            print('ATD ERROR: Error in {location}.{funct_name}() - line {line_no} : {error}'
    ...
    def submit_file(self):
        ...
        try:
            ...
            res = requests.post(self.url + "fileupload.php", headers=self.headers, files=files,
                              data={'data': json.dumps(data)}, verify=self.verify)
    ...
    def get_status(self, taskid):
        ...
        try:
            ...
            while status is True:
                ...
                res = requests.get(self.url + "samplestatus.php", params=payload, headers=self.headers, verify=self.verify)

                if res.status_code == 200:
                    state = res.json()['results']['istate']

                    if state == 1 or state == 2:
                        print('ATD STATUS: Analysis done with TaskID: {0}. Trying to get report.'.format(str(taskid)))
                        time.sleep(10)
                        status = False
                    else:
                        print('ATD STATUS: Analysis not done with TaskID: {0}. Status: {1}'.format(str(taskid), res.json()))
                        time.sleep(120)
                else:
                    print('ATD ERROR: Something went wrong in {0}. Error: {1}'.format(sys._getframe().f_code.co_name, str(res.text)))
                    sys.exit()
    ...
    def get_report(self, taskid):
        ...
        try:
            res = requests.get(self.url + "showreport.php", params=payload, headers=self.headers, verify=self.verify)
            if res.status_code == 200:
                #print(res.json())
    ...
    def logout(self):
        try:
            res = requests.delete(self.url + "session.php", headers=self.headers, verify=self.verify)
            if res.status_code == 200:
                print('ATD STATUS: Successful logged out {0}.'.format(ATD_USER))
            else:
                print('ATD ERROR: Something went wrong in {0}. Error: {1}'.format(sys._getframe().f_code.co_name, str(res.text)))
                sys.exit()
